...
{
	// 空間参照を取得
	if( FAILED (c_ipGeo->get_SpatialReference (&m_ipSpRef)) ) {
		Error_Print ( "Init_01", "空間参照の取得に失敗しました" );
		return		( false );
	}
...
//		InitRing( a_ipPtsCount, e_wpPoints, e_lRings );
		InitRing2( a_ipPtsCount, c_ipGeo, e_wpPoints, e_lRings );
		// 点列の包含関係をとってくる
								...
								long	c_lSubPart_Num	)	///< サブパート番号
{
	...
	// 空間参照を付与
	if( FAILED (a_ipRet_Geo->putref_SpatialReference (m_ipSpRef)) ) {
		Error_Print ( "Get_Geo_from_Part_Subpart_04", "空間参照の付与に失敗しました" );
		return		( false );
	}
	// 2010.06.09
	if( FAILED (a_ipRet_Geo->SnapToSpatialReference ()) ) {
		Error_Print ( "Get_Geo_from_Part_Subpart_04", "空間参照の付与に失敗しました" );
		return		( false );
	}
...
Get_Geo_from_Part	(	long	c_lPart_Num	)	///< パート番号
{
	...
	// 空間参照を付与
	if( FAILED (a_ipPart_Geo->putref_SpatialReference (m_ipSpRef)) ) {
		Error_Print ( "Get_Geo_from_Part_Subpart_04", "空間参照の付与に失敗しました" );
		return		( false );
	}
	// 2010.06.09
	if( FAILED (a_ipPart_Geo->SnapToSpatialReference ()) ) {
		Error_Print ( "Get_Geo_from_Part_Subpart_04", "空間参照の付与に失敗しました" );
		return		( false );
	}
	return	( a_ipPart_Geo );
...
#endif	
	// 空間参照を付与
	if( FAILED (a_ipNewPolygon->putref_SpatialReference (m_ipSpRef)) ) {
		Error_Print ( "Get_Part_Polygon_05", "空間参照の付与に失敗しました" );
		return		( false );
	}
	// 2010.06.10
	if( FAILED (a_ipNewPolygon->SnapToSpatialReference ()) ) {
		Error_Print ( "Get_Part_Polygon_05", "空間参照の付与に失敗しました" );
		return		( false );
	}
