...
#include "stdafx.h"
#include "AttrCitySiteDlg.h"

using namespace sindy::schema;

/// ウィンドウマネージャ配置マップ（都市地図背景用）
 ...
 *
 * @retval CAttrCitySiteDlgを返します。
 */
extern "C" __declspec(dllexport) CExportDlg * __stdcall CreateDlg()
{
    return new CAttrCitySiteDlg();
}

/////////////////////////////////////////////////////////////////////////////
//
// CAttrCitySiteDlg メンバ関数
//
/////////////////////////////////////////////////////////////////////////////

/** 
 * @fn CAttrCitySiteDlg::CAttrCitySiteDlg()
 * @brief コンストラクタ
 *
 * ブラシの初期化をしておきます。
 */
CAttrCitySiteDlg::CAttrCitySiteDlg() 
	: ATTR_BASE_CLASS(CSITE_MAP, IDD_ATTRCITYSITEDLG),
...
/** 
 * @fn HWND CAttrCitySiteDlg::Create(HWND hWndParent, LPARAM dwInitParam)
 * @brief 子ダイアログを作成します。
 *
 ...
 */
HWND CAttrCitySiteDlg::Create(HWND hWndParent, LPARAM dwInitParam)
{ 
...
/**
 * @fn void CAttrCitySiteDlg::Delete()
 * @brief 作成した子ダイアログを削除します。
 *
 * ダイアログを削除するときに必ず呼んで下さい。呼ばないとメモリリークしてしまいます。
 */
void CAttrCitySiteDlg::Delete()
{
	ClearFeatureDefs();
...
/**
 * @fn BOOL CAttrCitySiteDlg::SetCurrentFeatureDefIndex( LONG lFeatureIndex, LONG lTableIndex, LONG lRowIndex, BOOL bForce, BOOL bEditable )
 ...
 */
BOOL CAttrCitySiteDlg::SetCurrentFeatureDefIndex( LONG lFeatureIndex, LONG lTableIndex, LONG lRowIndex, BOOL bForce, BOOL bEditable )
{ 
...
/**
 * @fn void CAttrCitySiteDlg::CreateControlRelation()
 * @brief 子ダイアログを実際に作成します。
 *
 * 実際の作成はControlRelに任せます。
 */
void CAttrCitySiteDlg::CreateControlRelation()
{
	...
	if( m_cControlRel.size() > 0 )
	{
		...
		// 背景種別, 情報ソース
		if( m_strClassName == city_site::kTableName )
		{
			m_cControlRel.SetControl( IDC_COMBO_SITE,		TYPE_COMBO,		0,	0, city_site::kBgClass,	NULL, city_site::kTableName );
			m_cControlRel.SetControl( IDC_EDIT_SOURCE,		TYPE_EDITSTRING,0,	0, ipc_table::kSource,  NULL, city_site::kTableName );
		}
		else if( m_strClassName == city_site_under::kTableName )
		{
			m_cControlRel.SetControl( IDC_COMBO_SITE,		TYPE_COMBO,		0,	0,  city_site_under::kBgClass,	NULL, city_site_under::kTableName );
			m_cControlRel.SetControl( IDC_EDIT_SOURCE,		TYPE_EDITSTRING,0,	0,  ipc_table::kSource,         NULL, city_site_under::kTableName );
		}
...
/**
 * @fn BOOL CAttrCitySiteDlg::SetBackgroundClassCode(INT nCodeID, INT nClassID)
 ...
 */
BOOL CAttrCitySiteDlg::SetBackgroundClassCode(INT nCodeID, INT nClassID)
{
...
/**
 * @fn void CAttrCitySiteDlg::ShowGenkouByBackgroundClass()
 * @brief 現在選択中の背景種別の原稿番号を、原稿番号フィールドに表示
 */
void CAttrCitySiteDlg::ShowGenkouByBackgroundClass()
{
...
/**
 * @fn void CAttrCitySiteDlg::GetBackgroundClass(LONG *lBackgroundClass)
 * @brief 現在選択中の背景種別IDを取得
 *
 * @param	lBackgroundClass	[out]		選択中の背景種別ID		
 */
void CAttrCitySiteDlg::GetBackgroundClass(LONG *lBackgroundClass)
{
	CComboBox	cWAnnoClass(GetDlgItem(IDC_COMBO_SITE));
	LONG		lSelectIndex = cWAnnoClass.GetCurSel();
	*lBackgroundClass = (LONG)cWAnnoClass.GetItemData(lSelectIndex);
}

/**
 * @fn BOOL CAttrCitySiteDlg::CheckReturnKeyItem(INT nClassID)
 ...
 */
BOOL CAttrCitySiteDlg::CheckReturnKeyItem(INT nClassID)
{
...
/**
 * @fn BOOL CAttrCitySiteDlg::CheckEscKeyItem(INT nClassID)
 ...
 */
BOOL CAttrCitySiteDlg::CheckEscKeyItem(INT nClassID)
{
	if ((nClassID == IDC_EDIT_GENKOU) || (nClassID == IDC_BUTTON_SEARCH)) {		// 原稿番号エディットボックス or 検索ボタン
		if( !SetBackgroundClassCode(IDC_EDIT_GENKOU, IDC_COMBO_SITE) )
			return FALSE;
