...
public:
	...
	*/
	...
	/**
	*  @brief 選択しているグリッド行に対応する区画線ノードGIDを返す
	*  @param [out] compartNodeGid       区画線ノードGID
	*  @param [out] compartNodeGidNext   接続区画線ノードGID
	*  @param [in]  fieldSequence        フィールド名の順番
	*  @param [in]  fieldCompartNodeGID  フィールド名の区画線ノードGID
	*/
	void getSelectedNodeGid(CString& compartNodeGid, CString& compartNodeGidNext, const CString& fieldSequence, const CString& fieldCompartNodeGID) const;
...
protected:
	...
	CString m_linkGID;                          //!< グリッドで選択している区画線リンクGID
