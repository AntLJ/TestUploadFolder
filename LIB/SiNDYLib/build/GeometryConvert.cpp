...
// ジオメトリからマルチポイントを生成する
IGeometryPtr CGeometry::ToMultipoint( IGeometry* ipGeom )
{
...
// ジオメトリからポリラインを生成する
IGeometryPtr CGeometry::ToPolyline( IGeometry* ipGeom )
{
	IGeometryPtr ipRet;	// 返り値
	sindyErrCode emErr = sindyErr_NoErr;
...
// ジオメトリからパスを生成する
IGeometryPtr CGeometry::ToPath( IGeometry* ipGeom )
{
	IGeometryPtr ipRet;	// 返り値
	sindyErrCode emErr = sindyErr_NoErr;
	...
	LOGASSERTEERR_IF( ipGeom, sindyErr_ArgIsNull )
	{
		...
		switch( emType )
		{
			...
			case esriGeometryPath: // Path -> Path
				...
				break;
			default:
				LOGASSERTEERR_IF( SUCCEEDED( ipRet.CreateInstance( CLSID__Path ) ), sindyErr_COMCreateInstanceFailed )
				{
					IPointCollectionPtr ipRetCol( ipRet );	// 失敗することは無いはずなのでエラーチェックしない（面倒だし）
					ipRet->putref_SpatialReference( ipSpRef );
...
// ジオメトリからポリゴンを生成する
IGeometryPtr CGeometry::ToPolygon( IGeometry* ipGeom )
{
	IGeometryPtr ipRet;	// 返り値
	sindyErrCode emErr = sindyErr_NoErr;
...
// ジオメトリからリングを生成する
IGeometryPtr CGeometry::ToRing( IGeometry* ipGeom )
{
	IGeometryPtr ipRet;	// 返り値
	sindyErrCode emErr = sindyErr_NoErr;
...
IGeometryPtr CGeometry::Convert( IGeometry* ipGeom, esriGeometryDimension emGeomDim )
{
	IGeometryPtr ipRet;	// 返り値
	sindyErrCode emErr = sindyErr_NoErr;
