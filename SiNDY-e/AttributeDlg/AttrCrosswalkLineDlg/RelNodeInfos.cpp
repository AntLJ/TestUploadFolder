...
bool RelNodeInfos::read()
{
	...
	CString subFields = str_util::format( _T("%s,%s,%s"), 
								rel_lane_node_crosswalk_line::kObjectID, 
								rel_lane_node_crosswalk_line::kCrosswalkLineGID,
								rel_lane_node_crosswalk_line::kLaneNodeGID);
	...
	{
		CComVariant vaNodeGid = AheGetAttribute( row, rel_lane_node_crosswalk_line::kLaneNodeGID );
...
{
	using namespace sindy::schema::adam_v2::rel_lane_node_crosswalk_line;
	using namespace sindy::schema::adam_v2_adam_ipc_feature;
	using namespace uh;
	...
	if(! insertRecords.empty() )
	{
		for( const auto& rec : insertRecords )
		{
			...
			AheSetAttribute (ipBuffer, kCrosswalkLineGID, m_CrosswalkGid);
	...
	if(! deleteRecords.empty())
	{
		...
		CString where_clause = uh::str_util::format(_T("%s='%s' and %s in (%s)"), 
								kCrosswalkLineGID, m_CrosswalkGid,
