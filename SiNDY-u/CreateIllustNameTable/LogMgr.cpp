...
void LogMgr::writeOptionInfos( const boost::program_options::variables_map& varMap )
{
	for( auto rec : varMap )
	{
		CString strOption;
		strOption.Format( _T("    --%-20s %s"), CString( rec.first.c_str() ), CString( rec.second.as<std::string>().c_str() ) );
