 ...
 * CGeometryBaseクラスIPointCollectionインターフェース実装ファイル
...
// ジオメトリの総構成点数を取得する
long CGeometryBase::GetPointCount() const
{
	long lCount = -1;	// 返り値
...
// ジオメトリの構成点を取得する
CPoint CGeometryBase::GetPoint( long index ) const
{
...
// ジオメトリの構成点を取得する
CPoint CGeometryBase::GetPoint( long part, long index ) const
{
	IPointPtr ipPoint;	// 返り値
	...
	LOGASSERTE_IF( GetPartCount() > part, sindyErr_ArgLimitOver )
	{
		if( part == 0 )
			...
			CGeometryBase cGeom = GetPart( part );
...
// 構成点のイテレータを取得する
IEnumVertexPtr CGeometryBase::GetEnumVertex() const
{
...
sindyErrCode CGeometryBase::UpdatePoint( long i, IPoint* p )
{
