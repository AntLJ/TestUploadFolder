...
{
	if(S_OK != ipFields->FindField(CComBSTR(strFieldName), &lIndex)){
		CString strMsg;
		strMsg.Format(_T("%sの%のフィールドインデックスの取得に失敗しました"), strTableName, strFieldName);
		CLog::GetInstance().PrintLog(false, true, false, true, err_type::fatal, strMsg);
		return false;
	}
	return true;
...
bool GetSpatialReference(const CString& strTableName, IFeatureClassPtr ipFeatureClass, ISpatialReferencePtr& ipSpRef)
{
	...
	if(!ipGeoDataset){
		strMsg.Format(_T("IGeoDatasetへの変換に失敗"));
		CLog::GetInstance().PrintLog(false, true, false, true, err_type::fatal, strMsg);
		return false;
	}
	if(S_OK != ipGeoDataset->get_SpatialReference(&ipSpRef) || !ipSpRef){
		strMsg.Format(_T("空間参照取得失敗"));
		CLog::GetInstance().PrintLog(false, true, false, true, err_type::fatal, strMsg);
		return false;
	}
	return true;
...
{
	CString strMsg;
	if(S_OK != ipFeatureClass->GetFeature(lOID, &ipFeature)){
		strMsg.Format(_T("%sの OID:%ld のフィーチャ取得失敗"), strTableName, lOID);
		CLog::GetInstance().PrintLog(false, true, false, true, err_type::fatal, strMsg);
		return false;
	}
	return true;
...
{
	CString strMsg;
	if(S_OK != ipGeom->putref_SpatialReference(ipSpRef)){
		strMsg.Format(_T("%sの空間参照付与失敗"), strTableName);
		CLog::GetInstance().PrintLog(false, true, false, true, err_type::fatal, strMsg);
		return false;
	}
	return true;
...
bool GetOID(const CString& strTableName, IFeaturePtr ipFeature, long& lOID)
{
	...
	if(S_OK != ipFeature->get_OID(&lOID)){
		strMsg.Format(_T("%sのOID取得失敗"), strTableName);
		CLog::GetInstance().PrintLog(false, true, false, true, err_type::fatal, strMsg);
		return false;
	}
	return true;
