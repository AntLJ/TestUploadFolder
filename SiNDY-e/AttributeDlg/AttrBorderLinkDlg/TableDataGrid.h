...
public:
	...
	*/
	...
	/**
	*  @brief 選択しているグリッド行に対応する道路境界線ノードGIDを返す
	*  @param [out] borderNodeGid       道路境界線ノードGID
	*  @param [out] borderNodeGidNext   接続道路境界線ノードGID
	*  @param [in]  fieldSequence       フィールド名の順番
	*  @param [in]  fieldBorderNodeGID  フィールド名の道路境界線ノードGID
	*/
	void getSelectedNodeGid(CString& borderNodeGid, CString& borderNodeGidNext, const CString& fieldSequence, const CString& fieldBorderNodeGID)const;
...
protected:
	...
	CString m_linkGID;                          //!< グリッドで選択している道路境界線リンクGID
