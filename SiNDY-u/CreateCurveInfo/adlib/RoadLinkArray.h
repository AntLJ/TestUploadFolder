...
namespace adlib
{
	namespace road_link_array
	{
		...
		// TODO: "Array"だと順序付きの列っぽくなるので改名したい。
		/**
		 * @brief RowArrayをリンク用に特化したクラス
		 * @note	出典：SiNDYQToMPQ
		 */
		class RoadLinkArray : public RowArray
		{
		public:
			RoadLinkArray(void);
			~RoadLinkArray(void);
			...
			/**
			 * テスト用 RoadLinkArrayの中身を指定ストリームにダンプする
			 */
