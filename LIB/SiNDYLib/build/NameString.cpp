...
namespace
{
	...
	LPCTSTR FAILED_TO_GET_PROPERTYSET   = _T("Failed to get PropertySet");
	LPCTSTR CANNOT_GET_NAMEINFO         = _T("Becase the object do not have the interfece which is supported, it cannot get name information.");
#else
	LPCTSTR FAILED_TO_GET_PROPERTYSET   = _T("PropertySetの取得に失敗");
	LPCTSTR CANNOT_GET_NAMEINFO         = _T("サポート済みのインターフェースを持たないオブジェクトのため、名前情報は取得できません。");
...
HAS_WORKSPACE:
		if( NULL != ipWork )
		{
			...
			LOGASSERTEM_IF( NULL != ipProp, sindyErr_COMFunctionFailed, FAILED_TO_GET_PROPERTYSET )
				goto HAS_PROP;
		}
		else {
...
HAS_DATASET:
			if( NULL != ipDataset )
			{
				...
				LOGASSERTEM_IF( NULL != ipRow, sindyErr_ArgLimitOver, CANNOT_GET_NAMEINFO )
				{
					ITablePtr ipTable;
