...
void TableDataGrid::insert()
{
	...
	// バッファに属性セット(初期値)
	if(FAILED(ipBuffer->put_Value( AheFindField(fields, nq_border_node::kLastNodeF), CComVariant(0L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_border_node::kSourceUpdateC), CComVariant(-1L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_border_node::kUpdateType), CComVariant(1L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_border_node::kUserClaim), CComVariant(0L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_border_node::kBorderLinkGID), CComVariant(CComBSTR(m_linkGID)) ))) return;
	...
	for (const auto& field_col : m_gridField)
	{
		...
		if( field_col.m_field_name == nq_border_node::kObjectID)
			temp.push_back(_T(" "));
		else
...
{
	using namespace sindy::schema::adam_v2;
	if(fieldName != nq_border_node::kSequence) return false;
...
// 選択しているグリッド行に対応する道路境界線ノードGIDを返す
void TableDataGrid::getSelectedNodeGid(CString& borderNodeGid, CString& borderNodeGidNext, const CString& fieldSequence, const CString& fieldBorderNodeGID) const
{
	long selectedRow = m_grid->GetSelectedRow(0);
	if (selectedRow <= 0) return;

	borderNodeGid = grid_util::GetValue(m_grid, selectedRow, m_gridField.getCol(fieldBorderNodeGID)).bstrVal;
	if(borderNodeGid.IsEmpty()) return;
	...
	for( long indexRow = 1L; indexRow < rows; ++indexRow )
	{
		...
		{
			borderNodeGidNext = grid_util::GetValue(m_grid, indexRow, m_gridField.getCol(fieldBorderNodeGID)).bstrVal;
			break;
		}
	}
}
...
bool TableDataGrid::checkField( const IRowBufferPtr& ipRow, const CComVariant& vaValue, long row, long col, const CString& fieldName) const
{
	...
	if (type == esriFieldTypeSmallInteger || type == esriFieldTypeInteger) {
		if (!util::isInteger(vaValue) || (fieldName == nq_border_node::kSequence && CString(vaValue.bstrVal).IsEmpty())) {
