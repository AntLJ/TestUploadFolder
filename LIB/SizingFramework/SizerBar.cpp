...
BOOL CSizerBar::Create(DWORD dwStyle, HWND hParentWnd, CWinMgr& wm,
	UINT nID, const RECT& rc)
{
	m_pWinMgr = &wm;
#ifdef _AFXDLL
	CWnd* pWnd = CWnd::FromHandlePermanent(hParentWnd);
	ASSERT(pWnd);
	return CStatic::Create(NULL, dwStyle, rc, pWnd, nID);
#else
	BOOL bRet = ( baseClass::Create( hParentWnd, _U_RECT((LPRECT)&rc), NULL, dwStyle|SS_NOTIFY, 0, nID, NULL ) ) ? TRUE : FALSE;
	return bRet;
#endif // ifdef _AFXDLL
}
...
#else
BOOL CSizerBar::OnSetCursor(HWND/* hWnd*/, UINT/* nHitTest*/, UINT/* msg*/)
#endif // ifdef _AFXDLL
{
...
void CSizerBar::OnLButtonDown(UINT nFlags, CPoint pt)
{
	...
	ASSERT(pParentWnd);
#else
	CWindow cParentWnd( GetParent() );
	CWindow* pParentWnd = &cParentWnd;
	ASSERT(pParentWnd->IsWindow());
#endif // ifdef _AFXDLL
...
#else
	SendMessage(GetParent(), WM_WINMGR, nmr.idFrom, (LPARAM)&nmr);
#endif // ifdef _AFXDLL
}
...
#else
	::GetWindowRect(pParentWnd, &rcWin);
#endif // ifdef _AFXDLL
...
#else
	CBrush brush;//(reinterpret_cast<HBRUSH>(GetSysColor(COLOR_3DFACE)));
