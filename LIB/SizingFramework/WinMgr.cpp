...
// Window定義マップの変更を行う。
bool CWinMgr::ChangeMgrMap( WINRECT* winmap)
{
	m_map = winmap;
	WINRECT::InitMap(m_map);
...
#else
			CWnd cWnd = pWnd->GetDlgItem(wrc->GetID());
			CWnd* pChild = &cWnd;
#endif // ifdef _AFXDLL
			...
			if (wrc->IsWindow()) {
				...
				if (hwndChild) {
					...
					::DeferWindowPos(hdwp,
						...
						SWP_NOZORDER);
#else
					// コンボボックスの場合は高さのサイズ調整しない
					// MFC側ではこの対処はいらない
					TCHAR strClassName[6];
					::GetClassName( hwndChild, strClassName, 6 );
					if( lstrcmpi( strClassName, _T("COMBO") ) == 0 )
					{
						CRect cmbRect;
						GetWindowRect( hwndChild, &cmbRect );
						::DeferWindowPos(hdwp,
							hwndChild,
							NULL,		// HWND insert after
							rc.left,rc.top,rc.Width(),cmbRect.Height(),
							SWP_NOZORDER);
					}
					else {
						::DeferWindowPos(hdwp,
...
void CWinMgr::OnGetSizeInfo(SIZEINFO& szi, WINRECT* wrc, CWnd* pWnd)
{
	...
	if (wrc->IsGroup()) {
		...
		if( WRCT_PCT == wrc->Type())
		{
			...
			bRow = parent->IsRowGroup();

			pct = wrc->GetParam();
			ASSERT(0<pct && pct<100);
			hwTotal = bRow ? rcParent.Height() : rcParent.Width();
			hw = (hwTotal * pct) / 100;
			szi.szDesired = bRow ? CSize(szi.szDesired.cx, hw) : CSize(hw, szi.szDesired.cy);
		}
		...
		case WRCT_PCT:
			...
			hw = (hwTotal * pct) / 100;
			if( wrc->HasToFitSize()) {
				szi.szDesired = bRow ? CSize(wrc->GetHeightOrWidth( FALSE ), hw) :
					CSize(hw, wrc->GetHeightOrWidth( TRUE ) );
			}
			break;
...
void CWinMgr::TRACEDump(WINRECT* pWinMap) const
{
	...
	for (WINRECT* w=pWinMap; !w->IsEnd(); w++) {
		...
		CString s(' ',indent);
		if (w->IsGroup()) {
			s+=w->IsRowGroup() ? _T("ROWGROUP ") : _T("COLGROUP ");
