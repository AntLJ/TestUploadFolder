...
bool TableDataGrid::onAfterEdit(LONG Row, LONG Col)
{
	...
	if(value.IsEmpty() && (fieldName == nq_lane_node::kWidth ||
		fieldName == nq_lane_node::kCurvature ||
		fieldName == nq_lane_node::kCurvatureRadius ||
		fieldName == nq_lane_node::kCurvatureChangeRate ||
		fieldName == nq_lane_node::kClothoidParameter ||
		fieldName == nq_lane_node::kLongitudinalSlope ||
		fieldName == nq_lane_node::kCrossSlope)) 
		newVal.vt = VT_NULL;
...
void TableDataGrid::insert()
{
	...
	// バッファに属性セット(初期値)
	if(FAILED(ipBuffer->put_Value( AheFindField(fields, nq_lane_node::kLinearElement), CComVariant(-1L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_lane_node::kLastNodeF), CComVariant(0L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_lane_node::kSourceUpdateC), CComVariant(-1L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_lane_node::kUpdateType), CComVariant(1L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_lane_node::kUserClaim), CComVariant(0L)))
	|| FAILED(ipBuffer->put_Value( AheFindField(fields, nq_lane_node::kLaneLinkGID), CComVariant(CComBSTR(m_linkGID)) ))) return;
	...
	for (const auto& field_col : m_gridField)
	{
		...
		if( field_col.m_field_name == nq_lane_node::kObjectID)
			temp.push_back(_T(" "));
		else
...
{
	using namespace sindy::schema::adam_v2;
	if(fieldName != nq_lane_node::kSequence) return false;
...
// 選択しているグリッド行に対応する車線ノードGIDを返す
void TableDataGrid::getSelectedNodeGid(CString& laneNodeGid, CString& laneNodeGidNext, const CString& fieldSequence, const CString& fieldLaneNodeGID) const
{
	long selectedRow = m_grid->GetSelectedRow(0);
	if (selectedRow <= 0) return;

	laneNodeGid = grid_util::GetValue(m_grid, selectedRow, m_gridField.getCol(fieldLaneNodeGID)).bstrVal;
	if(laneNodeGid.IsEmpty()) return;
	...
	for( long indexRow = 1L; indexRow < rows; ++indexRow )
	{
		...
		{
			laneNodeGidNext = grid_util::GetValue(m_grid, indexRow, m_gridField.getCol(fieldLaneNodeGID)).bstrVal;
			break;
		}
	}
}
...
bool TableDataGrid::checkField( const IRowBufferPtr& ipRow, const CComVariant& vaValue, long row, long col, const CString& fieldName) const
{
	...
	if (type == esriFieldTypeSmallInteger || type == esriFieldTypeInteger) {
		if (!util::isInteger(vaValue) || (fieldName == nq_lane_node::kSequence && CString(vaValue.bstrVal).IsEmpty())) {
