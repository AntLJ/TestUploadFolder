...
// AttrMiddleSiteDlg.cpp: CAttrMiddleSiteDlg クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "AttrMiddleSiteDlg.h"

/// ウィンドウマネージャ配置マップ（ミドル背景ポリゴン用）
BEGIN_WINDOW_MAP(MIDDLE_SITE_MAP)
 ...
 *
 * @retval CAttrMiddleSiteDlgを返します。
 */
extern "C" __declspec(dllexport) CExportDlg * __stdcall CreateDlg()
{
    return new CAttrMiddleSiteDlg();
}

/////////////////////////////////////////////////////////////////////////////
//
// CAttrMiddleSiteDlg メンバ関数
//
/////////////////////////////////////////////////////////////////////////////

/** 
 * @fn CAttrMiddleSiteDlg::CAttrMiddleSiteDlg()
 * @brief コンストラクタ
 *
 * ブラシの初期化をしておきます。
 */
CAttrMiddleSiteDlg::CAttrMiddleSiteDlg()
	: ATTR_BASE_CLASS(MIDDLE_SITE_MAP, IDD_ATTRMIDDLESITEDLG),
...
/** 
 * @fn HWND CAttrMiddleSiteDlg::Create(HWND hWndParent, LPARAM dwInitParam)
 * @brief 子ダイアログを作成します。
 *
 ...
 */
HWND CAttrMiddleSiteDlg::Create(HWND hWndParent, LPARAM dwInitParam)
{ 
...
/**
 * @fn void CAttrMiddleSiteDlg::Delete()
 * @brief 作成した子ダイアログを削除します。
 *
 * ダイアログを削除するときに必ず呼んで下さい。呼ばないとメモリリークしてしまいます。
 */
void CAttrMiddleSiteDlg::Delete()
{
	ClearFeatureDefs();
...
/**
 * @fn BOOL CAttrMiddleSiteDlg::SetCurrentFeatureDefIndex( LONG lFeatureIndex, LONG lTableIndex, LONG lRowIndex, BOOL bForce, BOOL bEditable )
 ...
 */
BOOL CAttrMiddleSiteDlg::SetCurrentFeatureDefIndex( LONG lFeatureIndex, LONG lTableIndex, LONG lRowIndex, BOOL bForce, BOOL bEditable )
{ 
	int i=0;
...
/**
 * @fn void CAttrMiddleSiteDlg::CreateControlRelation()
 * @brief 子ダイアログを実際に作成します。
 *
 * 実際の作成はControlRelに任せます。
 */
void CAttrMiddleSiteDlg::CreateControlRelation()
{
	...
	if( m_cControlRel.size() > 0 )
	{
		...
		// S4の背景種別
		m_cControlRel.SetControl( IDC_COMBO_SC4BGCLASS_C,TYPE_COMBO,	0,	0, _T("SC4BGCLASS_C"),	NULL, MIDDLE_SITE_TABLE_NAME );
		// S4の原稿番号入力
		m_cControlRel.SetControl( IDC_EDIT_GENKOU4,		TYPE_EDIT,		0,	0 );
		m_cControlRel.SetControl( IDC_BUTTON_SEARCH4,	TYPE_BUTTON,	0,	0 );
		// S3の背景種別
		m_cControlRel.SetControl( IDC_COMBO_SC3BGCLASS_C,TYPE_COMBO,	0,	0, _T("SC3BGCLASS_C"),	NULL, MIDDLE_SITE_TABLE_NAME );
		// 以上同じにボタン
		...
		// S2の背景種別
		m_cControlRel.SetControl( IDC_COMBO_SC2BGCLASS_C,TYPE_COMBO,	0,	0, _T("SC2BGCLASS_C"),	NULL, MIDDLE_SITE_TABLE_NAME );
		// 以上同じにボタン
		...
		// S1の背景種別
		m_cControlRel.SetControl( IDC_COMBO_SC1BGCLASS_C,TYPE_COMBO,	0,	0, _T("SC1BGCLASS_C"),	NULL, MIDDLE_SITE_TABLE_NAME );
		// 以上同じにボタン
		...
		// 情報ソース
		m_cControlRel.SetControl( IDC_EDIT_SOURCE,		TYPE_EDITSTRING,0,	0, _T("SOURCE"),		NULL, MIDDLE_SITE_TABLE_NAME );
	}
}

/**
 * @fn BOOL CAttrMiddleSiteDlg::SetBackgroundClassCode(INT nCodeID, INT nClassID)
 * @brief 背景種別設定
 *
 ...
 */
BOOL CAttrMiddleSiteDlg::SetBackgroundClassCode(INT nCodeID, INT nClassID)
{
...
/**
 * @fn void CAttrMiddleSiteDlg::ShowGenkouByBackgroundClass()
 * @brief 現在選択中の背景種別の原稿番号を、原稿番号フィールドに表示
 */
void CAttrMiddleSiteDlg::ShowGenkouByBackgroundClass(INT nCodeID)
{
...
/**
 * @fn void CAttrMiddleSiteDlg::GetBackgroundClass(LONG *lBackgroundClass)
 * @brief 現在選択中の背景種別IDを取得
 *
 * @param	lBackgroundClass		[o]		選択中の背景種別ID		
 */
void CAttrMiddleSiteDlg::GetBackgroundClass(LONG *lBackgroundClass, INT nCodeID)
{
	INT cClassID = 0;
...
/**
 * @fn void CAttrMiddleSiteDlg::CopyValue(INT nOrgID, INT nCpyID)
 * @brief 下のスケールに原稿番号をコピーする
 *
 * @param	nOrgID		[in]		コピー元の背景種別コントロールID		
 * @param	nCpyID		[in]		コピー先の背景種別コントロールID		
 */
void CAttrMiddleSiteDlg::CopyValue(INT nOrgID, INT nCpyID)
{
...
/**
 * @fn BOOL CAttrMiddleSiteDlg::CheckReturnKeyItem(INT nClassID)
 ...
 */
BOOL CAttrMiddleSiteDlg::CheckReturnKeyItem(INT nClassID)
{
 ...
 */
BOOL CAttrMiddleSiteDlg::CheckEscKeyItem(INT nClassID)
{
	// 原稿番号エディットボックス or 検索ボタン
	if ((nClassID == IDC_EDIT_GENKOU1) || (nClassID == IDC_BUTTON_SEARCH1) ||
		(nClassID == IDC_EDIT_GENKOU2) || (nClassID == IDC_BUTTON_SEARCH2) ||
