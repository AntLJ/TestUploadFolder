...
class RefImpl
{
  ...
  bool operator == (const RefAny &refAnyR) const ;
  PObject *assignRef(PObject *objPtr,
                         PObject *srcObjPtr,
                         RefImpl *implPtr) ;
  PObject *assignObj(PObject *oldObjPtr,
                         PObject *newObjPtr) ;
...
{ 
   objptr = (T *)obj; 
   rimplPtr = new RefImpl((PObject *)objptr);
} 
...

{
    ...
    objptr = (src.objptr != NULL) ? (T *)(rimplPtr->pin()) :
                                    (T *)0;
}  
...
Ref<T>::~Ref() 
{ 
 // if exception is thrown , destruction cannot be
 // complete , hence not thrown
  try
  {
  rimplPtr->unpin((PObject *)objptr);
  }
  catch (SQLException &)
  {
  }
...
{ 
   ...
   {
     objptr = (T *)rimplPtr->assignRef((PObject *)objptr,
                                       (PObject *)src.objptr,
                                       src.rimplPtr);
     return *this;
   }
...
{ 
  if (objptr == obj)
    ...
    objptr = (T *)rimplPtr->assignObj((PObject *)objptr,
                        (PObject *)obj);
  return *this;
} 
...
const T* Ref<T>::operator->() const
{
  if (objptr == NULL)
  {
    const T *temp = (const T *)(rimplPtr->pin());
    return temp;
  }
  return ((const T *)objptr);
}
...
{
   rimplPtr->clear();
   /* if obj ptr is not null, then object un pin and then nullify */
   if (objptr)
     rimplPtr->unpin((PObject *)objptr);
   objptr = NULL;
}
