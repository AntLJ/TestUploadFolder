...
#include	"ArcHelperEx/define_enum.h" // TODO: 何とかしたいね
...
struct RING {
	...
	BOOL IsClockwise;
	std::list<SIMPLERING> Contain;	// このリングが含んでいるリングの番号リスト
	std::list<SIMPLERING> Within;	// このリングが含まれているリングの番号のリスト
	std::list<SIMPLERING> Cross;		// このリングとクロスしているリングの番号のリスト
	std::list<SIMPLERING> Disjoint;	// このリングと関連のないリングの番号のリスト
};
 ...
 *
 * @param START		[in]	RINGの開始インデックス
 * @param END		[in]	RINGの終了インデックス
 * @param x			[in]	検査する点のX値
 * @param y			[in]	検査する点のY値
 * @param pPoints	[in]	点列
 *
...
/**
 * @brief WKSPoint -> RING コンバータ
 * InitRingがfalseの場合に使用することを想定
 * ※InitRing()では自己接触ポリゴンに対応できない場合があるため追加
 *
 * @param lPointCount	[in]	総ポイント数
 * @param pPoints		[in]	ポイント配列
 * @retval TRUE 初期化成功
