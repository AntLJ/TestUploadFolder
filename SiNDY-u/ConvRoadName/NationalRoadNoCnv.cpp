...
bool NationalRoadNoCnv::findRoadName(const long& linkOID, const IFeaturePtr& ipLink, const long& subOID, const _IRowPtr& ipSubRow, RoadNameRecord& roadNameRecord)
{
	...
	roadNameKey.m_name.Format(_T("国道%s号"), num2ZenStr(vaRoadNo.lVal));
	roadNameKey.m_yomi.Format(_T("コクドウ%sゴウ"), num2YomiStr(vaRoadNo.lVal));
...
{
	// 道路名称分割データ シーケンス1登録
	SpltNameRecord spltNameRecord1(roadNameRecord.m_nameID, ::split_class::kBody, 1, _T("国道"), _T("コクドウ"));
	...
	name2nd = orgName.Mid(roadNameRecord.m_name.Find(_T('道'))+1);
	yomi2nd = orgYomi.Mid(roadNameRecord.m_yomi.Find(_T('ウ'))+1);
