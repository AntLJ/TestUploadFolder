...
bool PrefRoadNoCnv::setRoadLinkFieldIndex()
{
...
bool PrefRoadNoCnv::findRoadName(const long& linkOID, const IFeaturePtr& ipLink, const long& subOID, const _IRowPtr& ipSubRow, RoadNameRecord& roadNameRecord)
{
	...
	roadNameKey.m_name.Format(_T("%s道%s号"), prefCode2PrefName(linkPrefCodeVal.lVal), num2ZenStr(linkRoadNoVal.lVal));
	roadNameKey.m_yomi.Format(_T("%sドウ%sゴウ"), prefCode2PrefYomi(linkPrefCodeVal.lVal), num2YomiStr(linkRoadNoVal.lVal));
...
bool PrefRoadNoCnv::cnvSplitName(const long& linkOID, const IFeaturePtr& ipLink, const long& subOID, const _IRowPtr& ipSubRow, const RoadNameRecord& roadNameRecord, std::set<SpltNameRecord>& spltRdNameRecs)
{
	// 道路名称分割データ シーケンス1登録
