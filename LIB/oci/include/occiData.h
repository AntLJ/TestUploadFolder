...
class Bytes
{
 ...
 public:
  ...
  virtual void getBytes(unsigned char *dst, unsigned int count, 
                        unsigned int srcBegin = 0,
                        unsigned int dstBegin = 0) const; 
 
  virtual unsigned int length() const;
 
  virtual unsigned char byteAt(unsigned int index) const;
...
class Clob
{
  public:
    ...
    void trim(unsigned int newlen) ;
    Stream* getStream(unsigned int offset = 1,
      unsigned int amount =0, CharSet charsetId=DefaultCharSet,
      CharSetForm charsetForm = OCCI_SQLCS_IMPLICIT) ;
    void closeStream(Stream *stream);
...
class Number
{
 ...
 private:
  ...
  friend Number operator+(const Number &a,
   const Number &b) ;
  // dividend/divisor
  friend Number operator/(const Number &dividend,
  ...
  friend Number operator-(const Number &subtrahend,
...
class Timestamp
{
  ...
  private:
   ...
   Timestamp( OCIEnv *env, OCIDateTime *dt) ;
...
class IntervalDS
{
  ...
  IntervalDS( const Environment *env, const OCCI_STD_NAMESPACE::string &inpstr) ;
...
class IntervalYM
{
  ...
  IntervalYM( const Environment *env,const OCCI_STD_NAMESPACE::string &inpstr) ;
  ...
  friend const IntervalYM operator/(const IntervalYM &a,
  const Number &factor);
  friend bool operator==(const IntervalYM &a, 
  const IntervalYM &b) ;
  friend bool operator>(const IntervalYM &a, 
  const IntervalYM &b) ;
  friend bool operator<( const IntervalYM &a, 
