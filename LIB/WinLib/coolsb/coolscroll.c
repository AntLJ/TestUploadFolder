...
//
static void DrawImage(HDC hdc, HBITMAP hBitmap, RECT *rc, BOOL bUseMaskColor, COLORREF mask)
{
	BITMAP bm;
	int cx;
	int cy;   
	...
	if( ! bUseMaskColor )
		...
		TransparentBlt( hdc, rcDest.left, rcDest.top, rcDest.right-rcDest.left, rcDest.bottom-rcDest.top, memdc, p.x, p.y, rcDest.right-rcDest.left, rcDest.bottom-rcDest.top, mask );
	...
	case SBBT_PUSHBUTTON: 
		...
		else
		{
			...
			if(sbut->hBmp)
			{
				...
				IntersectClipRect(hdc, rect.left, rect.top, rect.right,rect.bottom);
				DrawImage(hdc, sbut->hBmp, &rect, sbut->bMaskColor, sbut->uMaskColor );
				SelectClipRgn(hdc, 0);
			}
			else if(sbut->hEmf)
			{
