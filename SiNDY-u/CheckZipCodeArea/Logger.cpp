...
void CErrLogger::writeStdLog(const CString& layer, const long oid, const CString& x, const CString& y,
	const CString& errLevel, const CString& errCode, const CString& errMsg, const CString& info)
{
	static const long flag = 0; // 対応状況

	// #対応状況 レイヤ名 オブジェクトID X Y エラーレベル エラーコード エラーメッセージ 情報 コメント
	Log(_T("%d\t%s\t%ld\t%s\t%s\t%s\t%s\t%s\t%s\n"),
		flag, layer, oid, x, y, errLevel, errCode, errMsg, info);
}
...
namespace log_utility
{
	...
	static uh::tstring GetLogfileNamePrefix()
	{
		...
		return uh::toTStr((uh::str_util::format(_T("%s_%s_"), strToolName, strTime)).GetString());
	}
