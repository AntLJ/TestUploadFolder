...
class SINDYLIB_API CGeometry : public CComWrapper<IGeometry>  
{
public:
	explicit CGeometry()
	{
...
//		Init();
	...
	explicit CGeometry( const CGeometry& obj )
	{
		Init();
		*this = obj;
	}

	CGeometry( IGeometry* lp )
	{
		Init();
		*this = lp;
	}
	explicit CGeometry( const GUID& guid )
	{
		IGeometryPtr lp;
	...
	{
		CopyCGeometry( *this, obj );
		return *this;
	}
	CGeometry& operator=( IGeometry* lp )
	{
	...
	{
		return ( IsCached() ) ? (IGeometry*)m_ipCache : (IGeometry*)p;
	}
	...
	operator IUnknown*() const { return ( IsCached() ) ? m_ipCache : p; }
	operator IUnknown*() { return ( IsCached() ) ? m_ipCache : p; }
	 ...
	 * <h3>例：</h3>
	 * @code
	 * const CGeometry* pGeom = cFeat.GetShape();
	...
	/**
	 * @brief <b>ジオメトリのキャッシュを作成する</b>\n
	 * 既にキャッシュが存在する場合はクリアされます。
	 * bDetatchをtrueにすると、キャッシュ作成後にIGeometry*
	 * はりリースされます。
	 * @param bDetatch	[in,optional]	キャッシュ作成後に切り離す場合はtrue
	 * return sindyErrCode
	 */
	errorcode::sindyErrCode CreateCache( bool bDetatch = false );

	/**
	 * @brief <b>ジオメトリがキャッシュされているかどうかを判断する</b>\n
	 * m_ipCacheがNULLかどうかで判断します。
	 * @retval true キャッシュ済み
	 * @retval false キャッシュされていない
	 */
	bool IsCached() const { if( NULL != m_ipCache ) return true; else return false; }

	/**
	 * @brief <b>ipGeomからipColのインターフェースを取得する</b>\n
	 * エラー処理と回数の関係上、別関数として定義されています。
	 * @param ipGeom	[in]	IGeometry*
	 * @param ipCol		[out]	IPointCollection*
	 * @retval true 成功
	 * @retval false 失敗
	 */
//	bool QueryPointCollection( IGeometry* ipGeom, IPointCollection** ipCol ) const;
	 ...
	 * @return sindyErrCode
	 */
	errorcode::sindyErrCode CheckLogic( const CBasicLogicRule& cRule, CErrorInfos& cErrInfos, IEnvelope* ipQuery = NULL ) const;
	 ...
	 * @return sindyErrCode
	 */
	errorcode::sindyErrCode CheckSegments( const CBasicLogicRule& cRule, CErrorInfos& cErrInfos, IEnvelope* ipQuery = NULL ) const;
	 ...
	 * @return sindyErrCode
	 */
	errorcode::sindyErrCode CheckPoints( const CBasicLogicRule& cRule, CErrorInfos& cErrInfos, IEnvelope* ipQuery = NULL ) const;
	//@}
#ifdef _DEBUG
	/**
	 * @brief <b>点列をトレースする</b>\n
...
private:
	 ...
	 * キャッシュがあれば、そのキャッシュは削除されます。\n
	 * このクラスを継承したクラスを作成する場合、コピーコンストラクタ
	 * を必ず定義し、その中でこの関数を呼ぶ必要があります。
	 */
	void Init();
	/**
	 * @brief <b>ジオメトリをコピーする</b>\n
	 * srcのをdestにコピーします。
	 * キャッシュも一緒にコピーされます。
	 * @param lp	[in]	CGeometry
	 * @return sindyErrCode
	 */
//	friend errorcode::sindyErrCode CopyGeometry( CGeometry& dest, const CGeometry& src );

	/**
	 * @brief <b>CGeometryをコピーする</b>\n
	 * @param dest		[in]	コピー先ジオメトリクラス
	 * @param src		[in]	コピー元ジオメトリクラス
	 */
	friend void CopyCGeometry( CGeometry& dest, const CGeometry& src );

private:
	// キャッシュ用
	IGeometryPtr m_ipCache;
};
#pragma warning(pop)
