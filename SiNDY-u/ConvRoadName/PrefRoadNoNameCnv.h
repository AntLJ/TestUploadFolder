...
public:
	 ...
	 */
	PrefRoadNoNameCnv(const std::map<CString, std::vector<CString>>& dictionary, const IFeatureClassPtr& ipRoadLink, const ITablePtr& ipRoadNameList, const ITablePtr& ipRoadCodeList, const ITablePtr& ipPrefRoadList)
		:PrefNameBaseCnv(dictionary, ipRoadLink, ipRoadNameList, ipRoadCodeList, ipPrefRoadList)
	{
		// 出力名称種別設定
		m_tgtNameClass = sindy::schema::sj::road_name_list::name_class::kPrefRoadNoName;
		// 道路リンク検索条件設定
