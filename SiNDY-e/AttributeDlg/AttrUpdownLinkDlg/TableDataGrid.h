...
public:
	...
	*/
	...
	/**
	*  @brief 選択しているグリッド行に対応する上下線ノードGIDを返す
	*  @param [out] updownNodeGid       上下線ノードGID
	*  @param [out] updownNodeGidNext   接続上下線ノードGID
	*  @param [in]  fieldSequence フィールド名の順番
	*  @param [in]  fieldUpdownNodeGID  フィールド名の上下線ノードGID
	*/
	void getSelectedNodeGid(CString& updownNodeGid, CString& updownNodeGidNext, const CString& fieldSequence, const CString& fieldUpdownNodeGID) const;
...
protected:
	...
	CString m_linkGID;                          //!< グリッドで選択している上下線リンクGID
	HWND m_hwnd;                                //!< グリッドコントロールのハンドル
	CComBSTR m_cellText;                        //!< セル情報
};
