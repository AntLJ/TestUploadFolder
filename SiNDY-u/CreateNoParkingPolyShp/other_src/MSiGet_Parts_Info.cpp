		...
		int				c_iMode	)		///< 再構成モードの設定（デフォルトは再構成しない）
{
	...
	//　壊さないようにする。
	IClonePtr	ipClone;
	if( FAILED( ((IClonePtr)c_ipGeo)->Clone( &ipClone ) ) ) {
		Error_Print ( "Init_00", "クローン作成に失敗" );
		return		( false );
	}
	...
	if( e_eGeoType != esriGeometryPolygon ) {	// ポリゴン以外のとき
		...
		double	tmpx, tmpy;
		IPointPtr	a_ipTmpPt;
		hr = a_ipTmpPts->get_Point ( 0, &a_ipTmpPt );
		if ( FAILED( hr ) ) {
			Error_Print ( "Init_02", "get_Pointに失敗" );
			return		( false );
		}
		hr = a_ipTmpPt->get_X( &tmpx );
		if ( FAILED( hr ) ) {
			Error_Print ( "Init_02", "get_Xに失敗" );
			return		( false );
		}
		hr = a_ipTmpPt->get_Y( &tmpy );	
		if ( FAILED( hr ) ) {
			Error_Print ( "Init_02", "get_Yに失敗" );
			return		( false );
		}
		...
		if ( e_wpPoints == NULL ) {
			Error_Print ( "Init_02", "WKSPointのメモリ確保に失敗" );
			return		( false );
		}
